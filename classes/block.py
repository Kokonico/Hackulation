...
class Liquid(Block):
    ...
    def __init__(self, x: int, y: int, appearance: str, viscosity: int, liquid_level: float = 100,
                 skip_tick: bool = False, surface_tension: float = 1.0):
    ...
    def tick(self, world):
        ...
        # LIQUID MOVEMENT 1. attempt to move down 2. if not possible, attempt to move to the sides 3. equalize the
        # liquid levels of the blocks to the sides (that means that the liquid level of this block will be the
        # average of the liquid levels of the blocks to the sides)
        ...
        if self.liquid_level > 0:
            # move to the sides
            if self.can_flow_into(self.x + 1, self.y, world, block_higher_level=True):
                blocks_to_move_to.append((self.x + 1, self.y))
            if self.can_flow_into(self.x - 1, self.y, world, block_higher_level=True):
                blocks_to_move_to.append((self.x - 1, self.y))
        ...
        if len(blocks_to_move_to) > 0:
            ...
            total_liquid = self.liquid_level
